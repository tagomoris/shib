...
YARNClient.prototype.status = function(jobname, callback){
  var yarn = this.yarn;
  var listCommand = yarn + " application -list -appStates ALL | grep " + jobname;
  child_process.exec(listCommand, function(err, stdout, stderr){
    ...
    var statusCommand = yarn + " application -status " + applicationId;
 ...
 */
      ...
      lines.forEach(function(line){
        ...
        switch (kv[0]) {
          ...
          case "Start-Time": status['startTime'] = new Date(parseInt(kv[1])).toLocaleString(); break;
        }
      });
