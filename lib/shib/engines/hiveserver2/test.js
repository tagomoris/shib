  ...
  this._inSession(function(err){
    ...
    client.GetSchemas(req, function(err, res){
      ...
      if (! res['operationHandle']) {
        console.log({message:"operationHandle missing for schemas"});
...
Executer.prototype.partitions = function(dbname, tablename, callback){
  ...
  if (dbname && dbname !== '')
    statement = 'show partitions ' + dbname + '.' + tablename;

  this.execute(null, null, statement, function(err, fetcher){
    if (err) { callback(err); return; }
    fetcher.waitInterval = OperationStatusPollingInterval;
    fetcher.fetch(null, function(err, result){
      if (err) { callback(err); return; }
...
Executer.prototype.partitions2 = function(dbname, tablename, callback){
  var self = this;
  var client = this._client;
