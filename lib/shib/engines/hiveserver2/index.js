...
'UKNOWN_STATE' : 6
        ...
        client.GetOperationStatus(req, function(err,res){
          if (err) { callback(err); return; }
          console.log(JSON.stringify({in:"waitComplete", res:res}, null, " "));
          // fetch operation fesult
          callback(null, new Fetcher(client, oph, self._maxRows));
        });
      });
    });
  });
...
'DECIMAL_TYPE' : 15
    ...
    switch (num) {
      ...
      case 14: return 'userdefined';
      case 15: return 'decimal';
    }
...
"results": {
 ...
 "columns": null
  ...
  // virtually blocks internally (GetOperationStatus doesn't works ...)
  this.fetch = function(num, callback){
    if (!num) { this._fetchAll(callback); return; }
    ...
    var fetchTry = function(){
      ...
      self._client.FetchResults(req, function(err, res){
        if (err) {
          if (res['status'])
            console.log(JSON.stringify({err:err, status:res.status}, null, " "));
          callback(err);
        ...
        if (!self._fetching && res.results.rows.length == 0) {
          // assumed that query still running
          console.log(JSON.stringify({op:'StillRunning?', status:res.status, rowsLength:fetchedLength, rows:fetchedRows}, null, " "));
          setTimeout(fetchTry, OperationStatusPollingInterval);
          return;
        }

        if (self._fetching && fetchedLength === 0) {
          // assumed to reach to end ?
          console.log(JSON.stringify({op:'FetchEnd?', status:res.status, rowsLength:fetchedLength, rows:fetchedRows}, null, " "));
          self._noMoreResults = true;
          self._closeOperation();
          callback(null, null);
          return;
        }
        console.log(JSON.stringify({op:'NowFetching', status:res.status, rowsLength:fetchedLength, rows:fetchedRows}, null, " "));
        self._fetching = true;
  ...
  // GetOperationStatus always returns res.status.statusCode == 0 and res.operationState == 2
  // fxxxxxxk!!!!!!!!!!!
 ...
 */
        ...
        if (statusCode === TTypes.TOperationState['CANCELED_STATE'])
          ...
          message = 'Query process ended with errror';
        else if (statusCode === TTypes.TOperationState['UNKNOWN_STATE'])
          message = 'Query status unknown';
        else
          message = 'Statement failed with unknown status:';
        callback({message:message, status:res.status});
      });
    };
    poller();
  };
