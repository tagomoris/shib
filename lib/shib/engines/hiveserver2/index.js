    ...
    this._waitComplete(function(err){
      ...
      */
      ...
      self._client.FetchResults(req, function(err, res){
        ...
        if (res.hasMoreRows !== null && res.hasMoreRows !== undefined && !res.hasMoreRows) {
          console.log(JSON.stringify({status:res.status, hasMoreRows:res.hasMoreRows, rows:res.results.rows.length}, null, " "));
 ...
 */
        ...
        if (statusCode !== TTypes.TOperationState['FINISHED_STATE']) {
          if (statusCode === TTypes.TOperationState['CANCELED_STATE'])
            callback({message:'Query canceled', status:res.status});
          else if (statusCode === TTypes.TOperationState['CLOSED_STATE'])
            callback({message:'Query closed', status:res.status});
          else if (statusCode === TTypes.TOperationState['ERROR_STATE'])
            callback({message:'Query process ended with errror', status:res.status});
          else if (statusCode === TTypes.TOperationState['UNKNOWN_STATE'])
            callback({message:'Query status unknown', status:res.status});
          else
            callback({message:'Statement failed with unknown status:' + ' (' + String(statusCode) + ')', status:res.status});
          return;
        }
        /* success complete */
        callback(null);
