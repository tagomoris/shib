...
var Client = exports.Client = function(args){
  ...
  console.log(this.conf.hiveserver.setup_queries);
  if (this.conf.hiveserver.setup_queries.length > 0) {
    var setups = this.conf.hiveserver.setup_queries.map(function(query){
      var q = query.trim();
      return q.trim() + (q.lastIndexOf(";") === q.length - 1 ? q : q + ";");
    });
    this.setup_queries = setups.join('');
  }
};
...
Client.prototype.getQueries = function(queryids, callback){
  ...
  this.getObjects(QUERY_KEY_PREFIX, queryids, function(err, data){
    if (err)
      ...
      callback.apply(client, [err, data.map(function(v){return new Query({json:v});})]);
  });
};
...
Client.prototype.giveup = function(query, callback){
  var resultid = query.results[query.results.length - 1].resultid;
  var client = this;
...
Client.prototype.execute = function(query, args){
  ...
  this.setResult(result, function(){
    ...
    var send_queries = this.setup_queries + query.composed();
    console.log("sending query:" + send_queries);
    client.hiveClient().execute(send_queries, function(err, data){
