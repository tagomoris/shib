...
Client.prototype.getObjects = function(type, keys, callback){
  ...
  this.kyotoClient().getBulk(objkeys, KT_SHIB_DEFAULT, function(err, data){
    if (err)
      ...
      callback.apply(this, [err, objkeys.map(function(k){return decodeObject(data[k]);})]);
  });
};
...
Client.prototype.getResults = function(resultids, callback){
  this.getObjects(RESULT_KEY_PREFIX, resultids, function(err, data){
    if (err)
      ...
      callback.apply(this, [err, data.map(function(v){return v && new Result({json:v});})]);
  });
};
...
Client.prototype.getLastResult = function(query, callback){
  if (query.results.length < 1){
    callback.apply(this, [undefined, null]);
    return;
  }
  this.getResults(query.results.reverse(), function(err, results){
    if (err){
      callback.apply(this, [err, undefined]);
      return;
    }
    var r;
    while((r = results.shift()) !== null){
      if (r.running)
        continue;
      callback.apply(this, [undefined, r]);
      return;
    }
    callback.apply(this, [undefined, null]);
  });
...
Client.prototype.status = function(query, callback){
   ...
   */
  if (query.results.length < 1) {
    callback.apply(this, ["waiting"]);
    return;
  }
  var resultid_revs = query.results.reverse();
  this.getResults(resultid_revs, function(err, results){
    if (! results.every(function(element, index, array){return element !== null;}))
      throw LocalStoreError("Result is null for one or more ids of: " + resultid_revs.join(","));

    var newest = results.shift();
    if (newest.running()){
      if (results.length < 1)
        callback.apply(this, ["running"]);
      else {
        var alter = results.shift();
        if (! alter.running() && ! alter.withError())
          callback.apply(this, ["cached"]);
        else
          callback.apply(this, ["running"]);
      }
    }
