...
Client.prototype.createQuery = function(querystring,req, callback){
  var client = this;
  // try {
  var seed = (new Date()).toTimeString(); // seed is not needed strictlicity
  var query = new Query({querystring:querystring, seed: seed,req:req});
...
Client.prototype.server_uri=function(){
    var executer_host=this._conf['executer']['host'];
    return executer_host;
}
...
Client.prototype.execute = function(query, args){
  ...
  var executed_at = (new Date()).toLocaleString();
    var result = new Result({queryid:query.queryid, executed_at:executed_at,name:"name",description:"description"});
