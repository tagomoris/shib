...
Client.prototype.giveup = function(query, delayedCallback, callback){
  var client = this;
...
Client.prototype.execute = function(query, args){
  ...
  this.setResult(result, function(){
    ...
    var engine = client.engine(query.engine);
    if (! engine) {
      result.markAsExecuted({message: "RELOAD page: unknown engine label, " + query.engine});
      if (args.error)
        args.error();
      return;
    }
