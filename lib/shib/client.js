...
Client.prototype.engine = function(label){
  ...
  if (! executer_conf.fetch_lines)
    executer_conf.fetch_lines = this._default_fetch_lines;

  var query_timeout = executer_conf.query_timeout;
  if (! query_timeout)
    query_timeout = this._default_query_timeout;

  if (! executer_conf.setup_queries)
    executer_conf.setup_queries = this._default_setup_queries;

  if (! executer_conf.default_database)
    executer_conf.default_database = this._default_default_database;

  this._engines[label] = new engine.Engine(label, executer_conf, monitor_conf, query_timeout);
  return this._engines[label];
};
...
console.log({p:"client.execute 1", query:query, args:args});
...
console.log({p:"client.execute 2", engine:query.engine, id:query.queryid, dbname:query.dbname, q:query.composed()});
...
console.log({p:"client.execute complete"});
      ...
      error: function(err){
console.log({p:"client.execute error", err:err});
...
console.log({p:"client.execute timeout", err:err});
        result.markAsExecuted(err);
        client.setResult(result);
        if (args.error)
          args.error();
...
console.log({p:"client.execute schema", error:error, schema:data});
...
console.log({p:"client.execute fetch", error:error, rows:rows});
