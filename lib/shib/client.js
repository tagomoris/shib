...
var Client = exports.Client = function(args){
  ...
  if (this._conf.hiveserver.support_database) {
      this._default_database = (this._conf.hiveserver.default_database || 'default');
  }
...
Client.prototype.engine = function(){
  ...
  this._engine = new engine.Engine(this._conf.executer, this._conf.monitor);
  return this._engine;
};
...
var jobName = function(queryid){ return 'shib(queryid):' + queryid; };
...
Client.prototype.executeSystemStatement = function(quoted_query, callback){
  var client = this;
  client.engine().execute('system statement', quoted_query, function(err, data){
...
Client.prototype.giveup = function(query, callback){
  ...
  client.engine().status(jobname, function(err, jobdata){
    //TODO: check status return value contents of huahin_mrv1 and huahin_yarn
    var jobid;
    if (jobdata)
      jobid = jobdata.jobid;

    if (client.engine().supports('kill') && jobid) {
      // killed job terminates hive query and executer calls callback with error "killed by user"
      client.engine().kill(jobid, function(err){ if (err) { console.log("Error on killing job:" + jobid); console.log(err); } });
      if (callback)
...
Client.prototype.execute = function(query, args){
  if (! args)
    args = {};
  //TODO: options not to add recent history
  ...
  this.setResult(result, function(){
    ...
    client.engine().execute(jobname, query.composed(), {
      setups: this.setup_queries,
      fetchNum: HIVESERVER_READ_LINES,
      ...
      fetch: function(error, rows){
        ...
        client.appendResultData(result.resultid, rows, function(err){
          if (err) { console.log(err); throw new LocalDiskStoreError("failed to append result data"); }
