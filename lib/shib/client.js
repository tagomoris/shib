...
Client.prototype.default_database = function(){
...
Client.prototype.createQuery = function(dbname, querystring, callback){
  var client = this;
  var seed;
  var query;
  try {
    seed = (new Date()).toTimeString(); // seed is not needed strictlicity
    query = new Query({querystring:querystring, dbname: dbname, seed: seed});
  }
  catch (e) {
...
Client.prototype.databases = function(callback){
  var client = this;
  if (! client.engine().supports('databases')) {
    callback(null, ['(default)']);
    return;
  }
  client.engine().databases(function(error, data){
    callback.apply(client, [error, data]);
  });
};

Client.prototype.tables = function(dbname, callback){
  var client = this;
  client.engine().tables(dbname, function(error, data){
    callback.apply(client, [error, data]);
  });
};

/* get partitions of specified table */
Client.prototype.partitions = function(dbname, tablename, callback){
  var client = this;
  client.engine().partitions(dbname, tablename, function(error, data){
...
Client.prototype.describe = function(dbname, tablename, callback){
  var client = this;
  client.engine().describe(dbname, tablename, function(error, data){
    callback.apply(client, [error, data]);
  });
};
...
Client.prototype.giveup = function(query, callback){
  ...
  client.engine().status(query.queryid, function(err, jobdata){
...
Client.prototype.execute = function(query, args){
  ...
  this.setResult(result, function(){
    ...
    client.engine().execute(query.queryid, query.dbname, query.composed(), {
...
Client.prototype.detailStatus = function(queryid, callback){
  var client = this;
  this.engine().status(queryid, function(err, status){
