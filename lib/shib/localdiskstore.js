...
LocalDiskStore.prototype.queries = function(queryids, callback){
  //TODO: check this code works correctly OR NOT
  this.db.all('SELECT json FROM queries WHERE id IN (?)', [queryids], function(err,rows){
    if (err) { callback(err); return; }
...
LocalDiskStore.prototype.results = function(resultids, callback){
  //TODO: check this code workds correctly OR NOT
  this.db.all('SELECT json FROM results WHERE id IN (?)', [resultids], function(err,rows){
    if (err) { callback(err); return; }
...
LocalDiskStore.prototype.recentQueries = function(num, callback){
  ...
  this.db.all(sql, function(err, rows){
    if (err) { callback(err); return; }
    //TODO: get queries and instanciate at here? or return [{yyyymm:.., queryid:...}] ?
    callback(null, rows);
  });
};
