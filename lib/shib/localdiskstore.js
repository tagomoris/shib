...
var LocalDiskStore = exports.LocalDiskStore = function(datadir) {
...
LocalDiskStore.prototype.addRecent = function(yyyymm, queryid, callback){
  this.db.run('INSERT INTO history (yyyymm,queryid) VALUES (?,?)', [yyyymm, queryid], function(err){
    if (callback) callback(err);
  });
};
LocalDiskStore.prototype.deleteRecent = function(queryid, callback){
  this.db.run('DELETE FROM history WHERE queryid=?', [queryid], function(err){
    if (callback) callback(err);
  });
};
...
LocalDiskStore.prototype.tagList = function(callback){
  this.db.run('SELECT DISTINCT tag FROM tags ORDER BY tag', function(err, rows){
    if (err) { callback(err); return; }
    callback(null, rows);
  });
};
LocalDiskStore.prototype.taggedQueries = function(tag, callback){
  this.db.run('SELECT queryid FROM tags WHERE tag=?', [tag], function(err, rows){
    if (err) { callback(err); return; }
    callback(null, rows);
  });
};
LocalDiskStore.prototype.tags = function(queryid, callback){
  this.db.run('SELECT tag FROM tags WHERE queryid=?', [queryid], function(err, rows){
    if (err) { callback(err); return; }
    callback(null, rows);
  });
};
LocalDiskStore.prototype.addTag = function(queryid, tag, callback){
  var self = this;
  this.db.run('SELECT queryid, tag FROM tags WHERE queryid=? AND tag=?', [queryid, tag], function(err, rows){
    if (rows.length > 0) {
