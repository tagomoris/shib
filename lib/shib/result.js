...
var Result = exports.Result = function(args){
  ...
  if (args.executed_time) { /* instanciated in code, with executed_time */
    ...
    if (! this.executed_msec && this.executed_at)
      this.executed_msec = new Date(this.executed_at).getTime();
  }
  ...
  if (args.completed_time) {
    ...
    if (! this.completed_msec && this.completed_at)
      this.completed_msec = new Date(this.completed_at).getTime();
  }
