...
var Engine = exports.Engine = function(label, executer_conf, monitor_conf, options) {
  ...
  this._acl = new AccessControl(executer_conf.access_control || {});
  ...
  if (monitor_conf) {
    ...
    case 'jobtracker':
      ...
      break;
      /*
       * see comment in hiveserver2/index.js
    case 'hiveserver2':
      monitor = hiveserver2.Monitor;
      break;
       */
  ...
  if (this.supports('tables')) {
    ...
    this._executer.tables(dbname, function(err, data){
      ...
      data.forEach(function(tablename){
        if (self._acl.allowed(tablename, dbname))
...
Engine.prototype.partitions = function(dbname, tablename, callback){
  if (this.supports('partitions')) {
    if (! this._acl.allowed(tablename, dbname)) {
...
Engine.prototype.execute = function(queryid, dbname, query, options) {
  ...
  table_db_pairs.forEach(function(pair){
    if (! self._acl.allowed(pair[0], (pair[1] || dbname)))
      access_denied = true;
  });
