...
Engine.prototype.kill = function(jobid, jobname, callback) {
  if (this.supports('kill')) {
    this._monitor.kill(jobid, jobname, callback);
  } else {
    callback({message:'Failed to kill job(jobid:' + jobid + ', jobname:' + jobname + '), not supported'});
  }
};
