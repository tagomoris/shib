var hive_service_types = require('gen-nodejs/hive_service_types'), // HiveClusterStatus, HiveServerException
...
var random_num = function(max){ return Math.floor(Math.random() * max); };
...
exports.cluster_status = function(){
  return hive_service_types.HiveClusterStatus({
    taskTrackers: 1,
    mapTasks: 0,
    reduceTasks: 0,
    maxMapTasks: 2,
...
var operator = function(num){
  return queryplan_types.Operator({
...
var operatorGraph = function(ops){
  ...
  for (var i = 0; i < ops.length - 1; i++){
    al.push(queryplan_types.Adjacency({ node: ops[i].operatorId, children: [ ops[i+1].operatorId ], adjacencyType: 0 }));
  }
  return queryplan_types.Graph({
    nodeType: 0,
    roots: null,
    adjacencyList: al
  });
...
exports.query_plan = function(querystring){
  var query = queryplan_types.Query({
    queryId: queryId(),
    queryType: null,
    queryAttributes: { queryString: querystring },
    queryCounters: null,
    stageList: [stage(1), stage(2)],
    stageGraph: queryplan_types.Graph({
      nodeType: 1,
      roots: null,
      adjacencyList: [ queryplan_types.Adjacency({ node: 'Stage-1', children: [ 'Stage-2' ], adjacencyType: 0 }) ]
    }),
    done: true,
    started: true
  });
  return queryplan_types.QueryPlan({ queries: [query], done: false, started: false });
};

var columns = function(query){
  return query.replace(/select (.*) from .*/i, "$1").split(/ *, */);
};
...
var columninfo = function(column){
  ...
  var match = /as ([_a-zA-Z0-9]*)$/i.exec(column);
  if (match){
    name = match[1];
  }
  if (/^count/i.exec(column)) {
    type = 'bigint';
    ex = 'count';
  }
  if (/id$/.exec(name)) {
...
exports.schema = function(query){
  var cols = columns(query);
  return {
    fieldSchemas: cols.map(function(c){
      var i = columninfo(c);
      return {name: i.name, type: i.type, comment: undefined};
    }),
    properties: null
  };
};
...
var generateValue = function(colinfo){
  ...
  case 'strcopy':
    return colinfo.name;
  case 'date':
    var d1 = new Date((new Date()).getTime() - Math.floor(Math.random() * 50) * 86400 * 1000);
    return '' + d1.getFullYear() + pad(d1.getMonth()+1) + pad(d1.getDate());
  case 'time':
    var d2 = new Date((new Date()).getTime() - Math.floor(Math.random() * 12 * 60) * 60 * 1000);
    return '' + pad(d2.getHour()) + pad(d2.getMinutes());
  case 'id':
